...
public final class Traverse {
    ...
    public static boolean to(GameAPI api, int tileX, int tileY, int plane) {
        ...
        return bresenhamTo(api, tileX, tileY, plane, useMinimap, stepSize);
    }

    public static boolean to(int tileX, int tileY, int plane) {
        return to(XApi.api(), tileX, tileY, plane);
    }

    public static boolean bresenhamTo(GameAPI api, int tileX, int tileY, int plane, boolean minimap, int stepSize) {
        LocalPlayer player = api.getLocalPlayer();
        if (player == null || player.plane() != plane) {
            return false;
        }
        ...
        if (distance > stepSize) {
            ...
            return walkTo(api, stepX, stepY, plane, minimap);
        }
        return walkTo(api, tileX, tileY, plane, minimap);
    }

    public static boolean bresenhamTo(int tileX, int tileY, int plane, boolean minimap, int stepSize) {
        return bresenhamTo(XApi.api(), tileX, tileY, plane, minimap, stepSize);
    }

    public static boolean walkTo(GameAPI api, int tileX, int tileY, int plane, boolean minimap) {
        LocalPlayer player = api.getLocalPlayer();
        if (player == null || player.plane() != plane) {
            return false;
        }
        ...
        if (distance > MAX_LOCAL_DISTANCE) {
            return bresenhamTo(api, tileX, tileY, plane, minimap, ThreadLocalRandom.current().nextInt(MIN_STEP_SIZE, MAX_STEP_SIZE));
        }
        api.queueAction(new GameAction(ActionTypes.WALK, minimap ? 1 : 0, tileX, tileY));
        return true;
    }

    public static boolean walkTo(int tileX, int tileY, int plane, boolean minimap) {
        return walkTo(XApi.api(), tileX, tileY, plane, minimap);
    }
}
