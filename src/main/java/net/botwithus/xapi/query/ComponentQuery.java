...
public class ComponentQuery implements Query<Component, ResultSet<Component>> {
    ...
    public ComponentQuery(GameAPI api, int... interfaceIds) {
        this.api = api;
    ...
    public static ComponentQuery newQuery(int... interfaceIds) {
        return new ComponentQuery(XApi.api(), interfaceIds);
    }

    public static ComponentQuery newQuery(GameAPI api, int... interfaceIds) {
        return new ComponentQuery(api, interfaceIds);
    }
    ...
    public ComponentQuery text(BiFunction<String, CharSequence, Boolean> matcher, String... text) {
        filter = filter.and(component -> {
            String value = api.getComponentText(component.interfaceId(), component.componentId());
            if (value == null) {
                return false;
            }
    ...
    public ComponentQuery option(BiFunction<String, CharSequence, Boolean> matcher, String... options) {
        filter = filter.and(component -> api.getComponentOptions(component.interfaceId(), component.componentId()).stream()
                .anyMatch(option -> {
    ...
    public ComponentQuery itemName(String name, BiFunction<String, CharSequence, Boolean> matcher) {
        filter = filter.and(component -> component.itemId() > -1
                && Boolean.TRUE.equals(matcher.apply(api.getItemType(component.itemId()).name(), name)));
        return this;
    }
